...
public class BaseErrorHandler {
    ...
    protected ApiErrorResponse mapToErrorResponse(HttpStatus httpStatus, MethodArgumentNotValidException exception, HttpServletRequest request) {
        return ApiErrorResponse.builder()
                .id(UUID.randomUUID().toString())
                .code("USR-001")
                ...
                .path(request.getRequestURI())
                .method(request.getMethod())
                .errors(extractAllValidationErrors(exception))
                .timestamp(TimeUtils.currentTime())
                .build();
